...
SUBROUTINE read_input()
  ...
  reflective_boundary = .FALSE. ! hard coded for Hypre
  ...
  tl_use_jacobi = .TRUE.
  verbose_on = .FALSE.
  ! PETSc and HYPRE stuff
  use_PETSC_kernels=.FALSE. 
  use_HYPRE_kernels=.FALSE.
  print_HYPRE_info=0
  
  halo_exchange_depth=2 ! Hard coded for Hypre
  ...
  ! Simple guess - better than a default of 10. Changed for 3D (JDS)
  if (tl_ppcg_inner_steps .eq. -1) then
    tl_ppcg_inner_steps = 4*INT(SQRT(SQRT(SQRT(REAL(grid%x_cells*grid%y_cells*grid%z_cells)))))
