...
SUBROUTINE tea_decompose(x_cells,y_cells,z_cells,left,right,bottom,top,back,front)
  ...
  INTEGER  :: lft,rght,tp,bttm,bck,frnt
  ...
  ! Allocate arrays containing the number of nodes in each cell along the x, y, and z coordinates
  ALLOCATE(lx(chunk_x))
  ALLOCATE(ly(chunk_y))  
  ALLOCATE(lz(chunk_z))    
    ...
    DO cy=1,chunk_y
      DO cx=1,chunk_x
        ...
        ENDIF  
          ...
          bck   = (cz-1)*delta_z+1+add_z_prev
          frnt  = bck+delta_z-1+add_z          
          lx(cx)=rght-lft+1
          ly(cy)=tp-bttm+1      
          lz(cz)=frnt-bck+1
